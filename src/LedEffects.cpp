...
/// LedEffect_Sparks
/////////////////////////////////////////
LedEffect_Sparks::LedEffect_Sparks(uint8_t theHue, float newSparkThreshold, float maxSpeed, float brightnessDecay)
{
    _TheHUE = theHue;
...
LedEffect_Sparks::~LedEffect_Sparks()
{
}

void LedEffect_Sparks::Draw(CRGBArray<NUM_LEDS>& theLeds)
{
    if (_IsFinished) {
        return;
    }
...
void LedEffect_Sparks::Advance()
{
    ...
    for (int i = 0; i < _numSparks; i++) {
        ...
        _TheSparks[i] *= _brightnessDecay;
        _TheSparksPos[i] += _TheSparks[i] * delta;

        if (_TheSparksPos[i] >= NUM_LEDS) {
            _TheSparksPos[i] = 0;
        }

        if (_TheSparksPos[i] < 0) {
